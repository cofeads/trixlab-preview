...
click('[data-tab="mats"]');
assert.equal(document.querySelector('[data-studio]').dataset.view,'mat-template','mats step opens the dedicated eight-piece layout');
assert.equal(document.querySelector('[data-mat-template-view]').hidden,false,'eight-piece traction mat board is visible');
assert.equal(document.querySelectorAll('[data-mat-cut-piece]').length,8,'all eight separate mat pieces are present');
click('[data-mat-view="rear"]');
...
click('[data-mat-colorway="race"]');
assert.equal(document.querySelector('[data-mat-color="bottom"]').value,'#ef1f2d','race mat preset applies the red reveal layer');
const matProfile=document.querySelector('select[data-mat-profile]');
const dualPrice=document.querySelector('[data-mats-only-price]').textContent;
matProfile.value='triple-9';
fire(matProfile,'change');
assert.equal(document.querySelector('[data-studio]').dataset.matProfile,'triple-9','9mm triple-layer mat profile activates');
assert.notEqual(document.querySelector('[data-mats-only-price]').textContent,dualPrice,'premium mat profile changes mats-only price');
const matBadge=document.querySelector('[data-mat-badge]');
matBadge.value='ck';
fire(matBadge,'change');
assert.equal(document.querySelector('[data-mat-preview]').textContent,'CK','CK badge renders on the traction mats');
click('[data-view="left"]');
...
fire(matsToggle,'change');
click('[data-mat-view="mat-template"]');
click('[data-export-mats-svg]');
assert.match(document.querySelector('[data-toast]').textContent,/1:1 MAT PRODUCTION PROOF READY/,'separate 1:1 traction mat export completes');
click('[data-add-mats]');
assert.equal(document.querySelector('[data-cart] b').textContent,'1','mats-only configuration adds to cart');
...
click('[data-add]');
assert.equal(document.querySelector('[data-cart] b').textContent,'2','configured kit adds beside the mats-only item');
assert.equal(JSON.parse(dom.window.localStorage.getItem('trixlab-cart-v1')).length,2,'both mats-only and configured kit persist for homepage checkout');
